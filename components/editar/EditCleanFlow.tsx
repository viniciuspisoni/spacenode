...
// EditCleanFlow — Editar v2 Clean (Gemini-first, controle manual).
//
// Direção (aprovada 2026-06-16): "controle manual na seleção, inteligência do
// Gemini na edição." O usuário contorna a área com ferramentas previsíveis
// (laço/polígono/pincel/borracha); o motor executa a edição e a recomposição
// preserva o restante. A detecção automática fica congelada como assist
// opcional, FORA do fluxo principal deste MVP.
//
// Três modos apenas: Trocar material · Remover / corrigir · Ajustar atmosfera.
//
// A tela mostra SÓ: imagem → modo → seleção → instrução → referência opcional →
// custo em Nodes → Gerar → antes/depois. Nada de provider/modelo/jargão — toda
// a complexidade vive em /api/edits/v2.
//
// Fase de testes: cobrança SIMULADA (a API não debita nodes); Alta precisão
...
// fora da UI). insert_element fica fora da UI do MVP (mantido no contrato).
type Mode = 'swap_material' | 'remove_element' | 'adjust_atmosphere'
...
interface ModeDef {
  ...
  label: string
  desc: string
  /** required = precisa de seleção; optional = funciona na imagem toda. */
  selection: 'required' | 'optional'
  hint: string
  placeholder: string
  refKind: 'material' | null
  refLabel?: string
}
...
const MODES: ModeDef[] = [
  {
    ...
    desc: 'Parede, piso, painel, bancada, marcenaria, tecido, pedra, madeira, metal ou revestimento.',
    selection: 'required',
    hint: 'Use o laço ou o polígono para contornar a superfície que vai receber o novo material.',
    placeholder: 'Ex.: trocar o painel por madeira clara',
    refKind: 'material',
    refLabel: 'Usar referência de material',
  },
  {
    id: 'remove_element',
    label: 'Remover / corrigir',
    desc: 'Objetos simples, manchas, artefatos, pequenas falhas e inconsistências.',
    selection: 'required',
    hint: 'Selecione a área que deseja remover ou corrigir. Para objetos, inclua brilhos e reflexos próximos.',
    placeholder: 'Opcional — ex.: remover este objeto',
    refKind: null,
  },
  {
    id: 'adjust_atmosphere',
    label: 'Ajustar atmosfera',
    desc: 'Iluminação, temperatura, contraste, exposição, clima e realismo — sem alterar a arquitetura.',
    selection: 'optional',
    hint: 'O ajuste vale para a imagem toda; a arquitetura, o mobiliário e os materiais são preservados. A seleção é opcional.',
    placeholder: 'Ex.: deixar a luz mais quente',
    refKind: null,
  },
]
...
export function EditCleanFlow({ initialBalance }: { initialBalance: number }) {
  const [step, setStep] = useState<Step>('empty')
  const [sourceUrl, setSourceUrl] = useState<string | null>(null)
  const [sourceDims, setSourceDims] = useState<{ w: number; h: number } | null>(null)
  const [mode, setMode] = useState<Mode>('swap_material')
  ...
  const needsMask = modeDef.selection === 'required'
  ...
  const needsInstruction =
    (mode === 'swap_material' && !referenceUrl) || mode === 'adjust_atmosphere'
  const canGenerate =
    !!sourceUrl &&
    busy === null &&
  ...
  const softWarning = (() => {
    if (!needsMask || !hasSelection) return null
      ...
      try {
        const res = await fetch('/api/edits/v2', {
          ...
          body: JSON.stringify({
            ...
            references:
              modeDef.refKind && referenceUrl
                ? [{ kind: modeDef.refKind, url: referenceUrl }]
                : [],
            dry_run: true,
            assume_mask: needsMask,
          }),
        })
        const json = await res.json().catch(() => null)
  ...
  }, [sourceUrl, step, mode, preservation, intensity, referenceUrl, modeDef.refKind, needsMask, instruction])
  ...
  }, [busy, instruction, intensity, mode, modeDef.refKind, needsMask, preservation, referenceUrl, sourceUrl, uploadFile])
  ...
  const segBtn = (active: boolean, isDisabled = false): React.CSSProperties => ({
    flex: 1,
    padding: '7px 0',
    fontSize: 12.5,
    borderRadius: 8,
    ...
    <div style={{ maxWidth: 1240, margin: '0 auto', padding: '28px 24px' }}>
      ...
      <div className="spn-editar-grid">
        ...
        <div style={{ ...card, display: 'flex', flexDirection: 'column', minHeight: 520 }}>
          {sourceUrl && (
            <EditV2Canvas
              ref={canvasRef}
              imageUrl={sourceUrl}
              tools={['lasso', 'polygon', 'brush', 'eraser', 'pan']}
              enableUndo
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* 1. Modo de edição */}
          <div style={card}>
            <div style={sectionLabel}>Modo de edição</div>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      setMode(m.id)
                      setError(null)
                      // A referência é específica de Trocar material; ao mudar
                      // para um modo sem referência, descarta a anexada.
                      if (!m.refKind) setReferenceUrl(null)
                    }}
                    style={{
                      textAlign: 'left',
                      padding: '9px 11px',
          ...
          {/* 2. Instrução  +  3. Referência (só Trocar material) */}
          <div style={card}>
          ...
          {/* 4. Preservação · 5. Intensidade · 6. Qualidade */}
          <div style={card}>
