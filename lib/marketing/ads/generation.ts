...
const FRONT_GUIDE: Record<CampaignFront, string> = {
  ...
  conversao:
    'conversao: fundo de funil/remarketing — oferta real e objetiva ("80 nodes grátis, sem cartão"; "planos a partir de R$ 99/mês"), fricção mínima, cta_button SIGN_UP.',
}
...
function buildSystemPrompt(rules: Record<string, unknown>): string {
  ...
  return [
    ...
    '4. Fatos reais permitidos sem conferência extra: "80 nodes grátis, sem cartão"; "planos a partir de R$ 99/mês"; "preserva geometria, proporções e perspectiva"; "aceita prints de SketchUp/Revit/Archicad como imagem base". Qualquer outro claim exige conferência humana — sinalize em risk_flags.',
