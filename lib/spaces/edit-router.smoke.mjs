...
// G7) Referência de textura SEM Vertex → NB2 (Vertex desligado = fallback NB2)
{
  const r = routeEdit(base({ tool: 'material', prompt: 'aplicar este porcelanato',
    references: [{ id: 't', url: 'https://x/tex.jpg', role: 'material_texture', source: 'upload' }] }))
  check('GF material+ref (Vertex off) → nano-banana-2', r.endpoint === 'fal-ai/nano-banana-2/edit', r)
  check('GF material+ref (Vertex off) → 2 nodes', r.costNodes === 2, r)
}

// G8) Vertex ligado → inpaint preciso para remove/material SEM referência (com crop)
//     + desconto de 1 node nos tiers pagos (localized 2→1, medium 3→2, global 4→3)
{
  process.env.VERTEX_IMAGEN_ENABLED = '1'
  // Endpoint e crop corretos
  const r = routeEdit(base())
  ...
  // Free fix inalterado (ainda 0)
  check('GF vertex free fix → 0 nodes', r.costNodes === 0, r)
  // Quick fix (cota usada) inalterado — já no piso de 1 node pago
  const rQ = routeEdit(base({ freeFixesUsedForImage: 1 }))
  check('GF vertex quick fix → 1 node (sem desconto no piso)', rQ.costNodes === 1, rQ)
  check('GF vertex quick fix → vertex/imagen-edit', rQ.endpoint === 'vertex/imagen-edit', rQ)
  // Localizado (não-Spacenode, máscara pequena, material) → 2→1
  const rL = routeEdit(base({ tool: 'material', isGeneratedBySpacenode: false, prompt: 'trocar piso' }))
  check('GF vertex localized (material) → 1 node (era 2)', rL.costNodes === 1, rL)
  check('GF vertex localized → vertex/imagen-edit', rL.endpoint === 'vertex/imagen-edit', rL)
  // Médio (máscara 30%, material sem ref) → 3→2
  const rM = routeEdit(base({ tool: 'material', maskAreaRatio: 0.30, isGeneratedBySpacenode: false, prompt: 'trocar parede' }))
  check('GF vertex medium (material) → 2 nodes (era 3)', rM.costNodes === 2, rM)
  // Global (máscara grande, remove) → 4→3
  const rG = routeEdit(base({ maskAreaRatio: 0.60 }))
  check('GF vertex global (remove large) → 3 nodes (era 4)', rG.costNodes === 3, rG)
  check('GF vertex global → vertex/imagen-edit', rG.endpoint === 'vertex/imagen-edit', rG)
  // Referência de textura COM Vertex on → Vertex (usa REFERENCE_TYPE_STYLE)
  ...
  check('GF vertex on + ref → vertex/imagen-edit (STYLE ref)', rRef.endpoint === 'vertex/imagen-edit', rRef)
  check('GF vertex + ref → 1 node (desconto Vertex + localized)', rRef.costNodes === 1, rRef)
  delete process.env.VERTEX_IMAGEN_ENABLED
}
