  ...
  try {
    ...
    if (debited && nodesToCharge > 0) {
      try {
        ...
        console.warn('[apresentar/moodboard] Refund:', nodesToCharge, 'nodes →', user.id)
      } catch (refundErr) {
        console.error('[apresentar/moodboard] FALHA NO REFUND:', { err: refundErr, userId: user.id, amount: nodesToCharge })
      }
    }
