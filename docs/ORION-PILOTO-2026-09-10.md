...
> **Atualização 2026-09-11:** Orion virou motor **público e pago** (20 nodes
> em 2K, 40 em 4K — cálculo em [[project_orion_gpt_image_25]]). O que muda:
> `canUseOrion` não checa mais `isInternalStaff` (só a flag
> `ORION_INTERNAL_ENABLED`), `is_internal_test` deixa de ser forçado
> true/nodes=0 (migration `20260911130000_renders_orion_public_pricing.sql`),
> variante/qualidade não são mais escolha do cliente (sempre Flare/high — ver
> `lib/orion/config.ts`), e a OpenAI entrou nas cláusulas 4-6 de
> `/privacidade`. O resto deste documento (contrato dos fornecedores, preset
> 2K/4K, medições de custo) continua valendo; as seções sobre autorização só
> de equipe e cobrança zero abaixo são **históricas**.
